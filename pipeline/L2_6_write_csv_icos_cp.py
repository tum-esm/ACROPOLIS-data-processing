...
from utils.import_data import import_manual_flagged_site_data
from utils.dataframe_operations import merge_L1_and_L2_manual_flag, convert_to_L2_1h_icos_cp_format
from utils.icos_cp_csv_conversion import df_to_L2_1min_icos_csv, df_to_L2_1h_icos_csv

from utils.paths import PIPELINE_DATA_DIRECTORY, CONFIG_DIRECTORY, LOG_DIRECTORY

assert (os.path.exists(PIPELINE_DATA_DIRECTORY))

# INFO: Script requieres *.paquet files to be manually generated and available in the pipeline/manual_flags directory.
...
logging.info("=========================================")
logging.info("Starting L2 *.paquet to *.csv conversion.")
...
for site in config["icos_cities_portal"]["site_names"]:
    ...
    df_to_L2_1min_icos_csv(df=df, sites_meta=sites_meta, site=site)
    
    # 1h data
    # Convert L2 1min DF to L2 1h DF
    df = df.pipe(convert_to_L2_1h_icos_cp_format) 
    
    # Convert DF to ICOS CP format (add additional columns, rename, etc.)
    df_to_L2_1h_icos_csv(df=df, sites_meta=sites_meta, site=site)
